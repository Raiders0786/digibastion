...
serve(async (req) => {
  ...
  try {
    ...
    const ipRateLimit = checkRateLimit(`unsub:ip:${clientIP}`, MAX_ATTEMPTS_PER_IP);
    ...
    const emailRateLimit = checkRateLimit(`unsub:email:${normalizedEmail}`, MAX_ATTEMPTS_PER_EMAIL);
